...
def test_power_sequence_resolves_dependency_order_and_delays() -> None:
    ...
    assert [step.domain_name for step in state.power_sequence] == ["supply", "logic"]
    assert [step.index for step in state.power_sequence] == [0, 1]
    assert state.power_sequence[0].delay == 0.002
    assert state.power_sequence[0].after is None
    assert state.power_sequence[1].delay == 0.005
    assert state.power_sequence[1].after == "supply"
...
def test_power_sequence_preserves_declaration_order_without_dependencies() -> None:
    ...
    state = ProjectGenerationProcessor().process(definition).device_states[0]
    assert [step.domain_name for step in state.power_sequence] == ["a", "b"]
    assert all(step.delay == 0.0 for step in state.power_sequence)
...
def test_power_off_sequence_defaults_to_reverse_power_on_order() -> None:
    ...
    assert state.power_sequence == state.power_on_sequence
