...
Power resources describe logical assignments such as `DC1`, `DC2`, and `DC3`. They are not hardware addresses.
...
Device-state definitions may use:
...
- fully explicit plans
- generated plans
- hybrid generation with explicit additions and overrides

An explicit plan should generally contain concrete values. Generated rules may contain references and series definitions that the compiler resolves.
...
Intermediate objects such as `GroupPartition`, `TestPlanCandidate`, and `PlannedDeviceState` should remain inspectable and independently testable.
